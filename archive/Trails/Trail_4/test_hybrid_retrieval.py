...
print("="*60)
print("INITIALIZING HYBRID NEURAL RETRIEVAL ENGINE")
print("="*60)
...
print("Loading FAISS Dense Index...")
faiss_index = faiss.read_index("index/faiss_combined.index")
...
splade_tokenizer = AutoTokenizer.from_pretrained("naver/splade-v3")
splade_model = AutoModelForMaskedLM.from_pretrained("naver/splade-v3", use_safetensors=False).to(device)
splade_model.eval()

# 2. BGE (Dense Semantic)
bge_model = SentenceTransformer("BAAI/bge-small-en-v1.5", device=device)

# 3. Cross-Encoder (Deep Reasoning)
cross_encoder = CrossEncoder("cross-encoder/ms-marco-MiniLM-L-12-v2", device=device)


# --- 2. The Sample JD (From LLM Parser) ---
# Notice: No hard constraints. We use mathematical penalties at the end.
jd_parsed = {
  "faiss_summary": "We are seeking an experienced Backend Engineer specializing in cloud-native Python and Java microservices. The candidate must have hands-on experience with modern data infrastructure including Spark, Airflow, and SQL warehouses, and be willing to relocate to Toronto. A strong educational background in computer science is required.",
...
dense_query = bge_model.encode([jd_parsed["faiss_summary"]], normalize_embeddings=True)
distances, indices = faiss_index.search(dense_query, 1000)
faiss_top_1000_idx = indices[0].tolist()
faiss_cids = df.iloc[faiss_top_1000_idx]['candidate_id'].tolist()

# B. Sparse Search (SPLADE Multi-Query)
...
# Get Top 1000 for each SPLADE chunk
splade_career_cids = df.iloc[np.argsort(score_career)[::-1][:1000]]['candidate_id'].tolist()
splade_skills_cids = df.iloc[np.argsort(score_skills)[::-1][:1000]]['candidate_id'].tolist()
splade_profile_cids = df.iloc[np.argsort(score_profile)[::-1][:1000]]['candidate_id'].tolist()
splade_edu_cids = df.iloc[np.argsort(score_edu)[::-1][:1000]]['candidate_id'].tolist()

print(f"Retrieval completed in {time() - start_time:.2f} seconds.")

# C. RRF Fusion
print("Fusing Dense and Sparse streams via RRF...")
fusion_lists = [
    faiss_cids, 
    splade_career_cids, 
    splade_skills_cids, 
...
top_500_cids = rrf(fusion_lists, k=60)[:500]

# --- 5. CROSS-ENCODER SCORING ---
print(f"Executing 4-Pass Cross-Encoder on Top {len(top_500_cids)} candidates...")
ce_start = time()

top_df = df[df['candidate_id'].isin(top_500_cids)].copy()
...
exp_penalty = (exp_deficit > 0).astype(float) * 0.15  # -15% if they don't meet min experience
...
print("\n" + "="*60)
print(f"TOP 5 CANDIDATES FOR: Backend Engineer")
print("="*60)

for i, (_, row) in enumerate(final_ranking.head(5).iterrows()):
    ...
    print(f"  [Career Snippet]: {row['text_career'][:100]}...")
