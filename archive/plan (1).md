...
## 12. Post-implementation debugging log

**Storage: fixed correctly — ~4.75GB, matches §4.** Offline enforcement, RRF, and non-thinking mode (confirmed by the empty `<think>\n</think>` in the logged output) are all working as designed.

**Time: got worse, not better — 1690.98s (28.2 min) vs the original 20+ min.** That's the important signal. Quantization alone doesn't buy you speed if every call is still issued one at a time — it only buys you storage and a lower ceiling per call. The thing §6 flagged as "the one to actively engineer, not just quantize" — batching — wasn't implemented yet, and the numbers show it:

| Stage | Time | % of total | Diagnosis |
|---|---|---|---|
| Dense embed + FAISS search | 12.3s | 0.7% | Suspicious on its own — FAISS search over any size index you're running is sub-second. This is almost certainly the *embedding call*, not the search. Instrument the two separately; check `n_ctx` on the embedding model isn't set to something huge (e.g. the model's 32k max) for a one-line JD query — large `n_ctx` can add fixed allocation overhead per call. |
| SPLADE + RRF | 5.0s | 0.3% | Roughly fine, not a priority. |
| Cross-encoder, 100 pairs | 173.8s | 10.3% | **1.74s/pair.** An INT8, 512-token-capped, 568M-param single forward pass should be tens to low hundreds of ms on CPU. This is the signature of 100 separate `session.run()` calls in a loop instead of one batched call. Also worth a quick check: does `bge-reranker-v2-m3-int8.onnx` on disk actually measure ~0.6GB? If it's still close to 2GB, the INT8 quantization didn't fully apply and you're paying fp32 compute cost regardless of the filename. |
| **LLM rationales, 100 candidates** | **1472.4s** | **87.1%** | **14.72s/candidate.** Your rationales run ~50-60 tokens, which implies roughly **3.5-4 tok/s** — abnormally slow for a Q4_K_M 1.7B model on llama.cpp; a correctly-threaded setup on ordinary modern CPU hardware typically clears 10-20+ tok/s single-stream. This is the dominant cost by a wide margin (87% of total wall-clock) and the only stage worth fixing before anything else. |

**Before swapping the model:** a different model run through the same one-call-at-a-time loop will hit the same wall. Going from 1.7B to something smaller is roughly a 2-3x lever at best. Fixing batching is realistically a 8-10x+ lever on the LLM stage alone, and it's a code change, not a model change. Do this first, then decide if you still need a smaller model on top of it.

### Fix 1 — Cross-encoder: batch the ONNX calls
```python
# instead of looping session.run() once per candidate:
pairs = [(jd_text, c.text) for c in top_100]
enc = tokenizer(pairs, padding=True, truncation=True, max_length=512, return_tensors="np")
outputs = bge_session.run(None, {
    "input_ids": enc["input_ids"],
    "attention_mask": enc["attention_mask"],
    # + "token_type_ids" if the exported graph expects it
})
scores = outputs[0].squeeze(-1)   # all 100 in one call (chunk into batches of 16-32 if you hit a memory ceiling)
```
Also explicitly set `sess_options.intra_op_num_threads` to your physical core count when creating the `InferenceSession` — an unset value doesn't always pick the optimal thread count.

### Fix 2 — LLM: run llama-server with continuous batching instead of looping
`llama-cpp-python`'s high-level `create_completion()` is single-sequence; real batching needs the actual llama.cpp server, which is still 100% local/offline (localhost only, no external network):
```bash
# start once, as part of your persistent service startup
llama-server -m models/qwen3-1.7b-q4_k_m.gguf --parallel 8 --cont-batching -c 8192 -t <physical_cores>
```
```python
import concurrent.futures, requests

def get_rationale(candidate):
    r = requests.post("http://localhost:8080/v1/chat/completions", json={
        ...
        "max_tokens": 60,
        "temperature": 0.7, "top_p": 0.8, "top_k": 20,   # non-thinking mode's recommended sampling settings
    })
    return r.json()["choices"][0]["message"]["content"]

with concurrent.futures.ThreadPoolExecutor(max_workers=8) as pool:
    rationales = list(pool.map(get_rationale, top_100))
```
Target: with a handful of parallel slots, aggregate throughput on modest CPU hardware should realistically reach 60-100+ tok/s, which puts ~5,500 total output tokens (100 candidates × ~55 tokens) at roughly **60-90 seconds** instead of 1472.

Two things worth checking regardless of the above: (1) confirm your llama.cpp build actually reports `AVX2 = 1` (or better) in its startup log — a non-optimized build on Windows can silently run much slower; (2) run `llama-bench` once against the GGUF file to get a raw single-stream tok/s ceiling for your exact hardware, so you know what's achievable before tuning further.

### On swapping Qwen3-1.7B for something smaller
Only worth revisiting *after* the fixes above — but if you still want a smaller/faster option at that point, in rough order of fit for short, grounded, templated rationale generation on CPU:

| Model | Why |
|---|---|
| **Qwen3-0.6B** (non-thinking) | Same family/tooling you already have working, ~3x fewer params. Reasonable choice if you want minimal code change. |
| **Qwen2.5-1.5B-Instruct** | Not a hybrid model — no thinking mode at all, so no `<think>` tokens or occasional reasoning-leakage to worry about (a known, measured behavior in hybrid Qwen3 models even with `/no_think`). Comparable size/speed to your current model, simpler behavior. |
| **Llama-3.2-1B-Instruct** | Smaller still, and one of the most heavily optimized small models in llama.cpp specifically — mature quantizations, well-tested CPU performance. |

One more thing worth a look independent of speed: R2 and R3 in your log are nearly identical in structure and phrasing ("The candidate's extensive experience in building production-grade embeddings-based retrieval systems, combined with...makes them a [top/strong] fit for the Senior AI Engineer role at Redrob AI"). Once generation is fast enough to iterate quickly, it's worth checking whether rationales are differentiating candidates enough, or converging on a template — a small amount of prompt variation (e.g. explicitly asking it to name the *specific* distinguishing skill/experience) usually fixes this.
