...
try:
    ...
    logging.warning("Missing dependencies. Run: uv add sentence-transformers faiss-cpu pandas psutil")
...
class RankingPipeline:
    ...
    def load_artifacts(self):
        ...
        logging.info("Loading Cross-Encoder (ms-marco-MiniLM-L-12-v2)...")
    ...
    def retrieve(self, top_k: int = 100) -> pd.DataFrame:
        ...
        self.log_metric("Dense Retrieval (FAISS)", start_time, start_mem)
        return retrieved_df
    ...
    def run(self, output_csv: str = "Trails/Trail_3/submission.csv"):
        total_start = time.time()
        
        self.load_artifacts()
        
        # 1. Retrieval
        retrieved_df = self.retrieve(top_k=5000) # Fetch top 5000 for Cross-Encoder
        ...
        # Output Metrics
        with open("Trails/Trail_3/metrics_report.txt", "w") as f:
            f.write("=== Trail 3 Resource Profile (L-12-v2) ===\n")
            for m in self.metrics:
                f.write(m + "\n")
            f.write(f"\nTotal Pipeline Execution: {time.time() - total_start:.2f} seconds\n")
            
        logging.info(f"Pipeline finished in {time.time() - total_start:.2f} seconds. Output saved to {output_csv}")

if __name__ == "__main__":
    pipeline = RankingPipeline()
    pipeline.run("Trails/Trail_3/submission.csv")
