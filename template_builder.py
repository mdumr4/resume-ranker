...
def build_templates(jsonl_path="candidates.jsonl", output_parquet="index/features_v2.parquet"):
    ...
    with open(jsonl_path, "r", encoding="utf-8") as f:
        for i, line in enumerate(f):
            ...
            # Career Gap Calculation - Merged Timeline Approach
            intervals = []
            
            # Extract education intervals
            for ed in edu:
                sy = ed.get('start_year')
                ey = ed.get('end_year')
                if sy is not None and ey is not None:
                    try:
                        intervals.append((int(sy), int(ey)))
                    except Exception:
                        pass
                        
            # Extract career intervals
            for exp in ch:
                ...
                if sd:
                    s_year = sd.year
                    e_year = MAX_SYSTEM_DATE.year if (is_curr or not ed) else ed.year
                    intervals.append((s_year, e_year))
            ...
            if intervals:
                ...
                # Apply dynamic penalty (max -0.45 weight)
                gap_penalty = min(total_gap, 15) / 15 * 0.45
